...
import { createConverter, DebugExporter, SourceApiReader } from '@avocode/octopus-fig/lib/src/index-node'
...
export async function convertDesign({
  ...
  exporter.on('octopus:component', async (result: ConvertedDocumentResult, role: string) => {
    ...
    console.info(`  ${chalk.cyan(`Octopus:`)} file://${result.octopusPath}`)
  ...
  await converter.convertDesign({
    designEmitter: reader.parse(),
    exporter,
    skipReturn: true,
  })
